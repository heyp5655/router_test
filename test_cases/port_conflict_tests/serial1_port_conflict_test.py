...
测试内容：
跳转到Serial1配置页面，测试Modbus、TCP Server、UDP Server这3个协议
...
测试流程：
1. 登录路由器Web界面
2. 跳转到Serial1配置页面 (#industrial/serialport/serial1)
3. 启用Serial1并选择DTU Mode
...
参考实现：D:\\WK\\Milesight\\AUTO\\port_check.py 的 configure_Serial1_ports()
"""
...
class Serial1PortConflictTest(BaseTest):
    """Serial1端口冲突检测测试（ID13）"""
    ...
    def __init__(self, config):
        ...
        # 从配置文件加载保留端口列表
        if Serial1PortConflictTest.RESERVED_PORTS is None:
            Serial1PortConflictTest.RESERVED_PORTS = self._load_reserved_ports()
    ...
    def setup(self):
        ...
        # 备份当前配置
        self.original_config = self.router_client.backup_serial_config(1)
    ...
    def execute(self):
        ...
        # 调用RouterClient的测试方法
        stats = self.router_client.configure_Serial1_ports(self.RESERVED_PORTS)
    ...
    def cleanup(self):
        ...
        try:
            ...
            if hasattr(self, 'original_config') and self.original_config:
                print("恢复Serial1配置...")
                if self.router_client.restore_serial_config(1, self.original_config):
                    print("✅ 配置已恢复到测试前的状态")
                else:
