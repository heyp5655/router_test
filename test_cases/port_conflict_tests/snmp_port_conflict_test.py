...
测试流程：
1. 登录路由器Web界面
2. 跳转到SNMP配置页面 (#industrial/snmp/snmp)
3. 启用SNMP
4. 填写基本信息（Location和Contact）
5. 遍历保留端口：
   - 输入端口号 → 保存 → 检测是否有弹窗
   - 有弹窗 = 通过，无弹窗 = 失败
6. 输出详细的测试统计报告

参考实现：D:\\WK\\Milesight\\AUTO\\port_check.py 的 configure_snmp_ports()
"""
...
class SNMPPortConflictTest(BaseTest):
    """SNMP端口冲突检测测试（ID16）"""
    ...
    def __init__(self, config):
        ...
        # 从配置文件加载保留端口列表
        if SNMPPortConflictTest.RESERVED_PORTS is None:
            SNMPPortConflictTest.RESERVED_PORTS = self._load_reserved_ports()
    ...
    def setup(self):
        ...
        # 备份当前配置
        self.original_config = self.router_client.backup_snmp_config()
    ...
    def execute(self):
        ...
        # 调用RouterClient的测试方法
        stats = self.router_client.configure_snmp_ports(self.RESERVED_PORTS)
    ...
    def cleanup(self):
        ...
        try:
            ...
            if hasattr(self, 'original_config') and self.original_config:
                print("恢复SNMP配置...")
                if self.router_client.restore_snmp_config(self.original_config):
                    print("✅ 配置已恢复到测试前的状态")
                else:
