...
# 修改 Phase 2 为 AES256 + SHA256
RACOON_CONFIG = """#
# Racoon配置 - Phase 2: AES256 + SHA256
# 更新时间: 2025-11-27
#
...
sainfo anonymous {
    ...
    encryption_algorithm aes256;       # Phase 2: AES256
...
try:
    print('=' * 60)
    print('修改 Phase 2 SA算法为 AES256 + SHA256')
    print('=' * 60)
    print()
    ...
    print('写入配置...')
    sftp = ssh.open_sftp()
    with sftp.file('/tmp/racoon_aes256.conf', 'w') as f:
        f.write(RACOON_CONFIG)
    sftp.close()

    run_sudo_command(ssh, 'mv /tmp/racoon_aes256.conf /etc/racoon/racoon.conf')
                ...
                for j in range(1, min(5, len(lines) - i)):
                    if lines[i+j].strip() and ('esp' in lines[i+j] or 'aes' in lines[i+j] or 'hmac' in lines[i+j]):
                        ...
                        if 'aes-cbc' in lines[i+j]:
                            print(f'  ℹ️  使用的加密算法: AES-CBC (AES256)')
